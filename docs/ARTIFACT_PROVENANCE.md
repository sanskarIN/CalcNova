...
The generator reads the restored .NET/NuGet `project.assets.json` for the platform project and emits deterministic **CycloneDX 1.5** JSON. The document records:
...
Regression source in `tools/tests/test_validate_release_workflow.py` locks the same SBOM/permission/action/checksum/subject contract. Generator behavior is separately locked by `tools/tests/test_generate_sbom.py`.
