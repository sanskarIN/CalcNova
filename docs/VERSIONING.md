...
**CalcNova 2.9.6** is the public/product release version.

The current 2.9.6 release uses strict Semantic Versioning-compatible numeric components, so the public/product and .NET/NuGet versions are identical:

| Purpose | Value |
| --- | --- |
| Product/display version | `2.9.6` |
| .NET/NuGet package version | `2.9.6` |
| Normalized Git release tag | `v2.9.6` |
| Assembly version | `2.9.6.0` |
| File version | `2.9.6.0` |
| Informational version | `2.9.6` |
| Android/iOS numeric build code | `20906` |

Earlier 2.9-series checkpoints are preserved in [`releases/2.9.0.md`](releases/2.9.0.md) and [`releases/2.9.5.md`](releases/2.9.5.md).
...
- `Version`, `VersionPrefix`, and `PackageVersion` = `2.9.6`;
- `AssemblyVersion` and `FileVersion` = `2.9.6.0`;
- `InformationalVersion` = `2.9.6`.

Android and iOS use `$(ProductDisplayVersion)` for their visible application version and `20906` for the platform numeric build code.
...
```text
ApplicationDisplayVersion = 2.9.6
ApplicationVersion = 20906
```
...
Linux AppStream metadata preserves stable entries for 2.8.03, 2.9.0, 2.9.5, and the current 2.9.6 release. The packaging validator requires exactly one stable AppStream entry for the current source display version and validates that entry's release date and description.
