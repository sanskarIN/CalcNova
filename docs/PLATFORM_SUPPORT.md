# CalcNova 1.0.0 Platform Support

## Status

**Cross-platform source composition is complete for CalcNova 1.0.0.**
...
`PROJECT_STATE.md` is the authoritative 1.0.0 completion record.
...
- 1.0.0 release identity inherited from central build properties.
...
- `.desktop` metadata;
- AppStream metadata with a single stable 1.0.0 release entry, because no earlier version was published;
- package identity validation.
...
The mobile build version required by this validator is derived from `Directory.Build.props` through `tools/release_identity.py`, preventing stale hardcoded build-code expectations during future release bumps.
...
The public CalcNova release version is `1.0.0`.
...
Do not convert an unexecuted target operation into PASS because source or a workflow exists. Conversely, `NOT RUN` in one environment does **not** mean the 1.0.0 platform source composition is unfinished.

## Operational platform follow-up

No additional platform head is required for the defined CalcNova 1.0.0 product scope. Operational or optional follow-up includes:
...
- 1.0.0 platform version identity: **COMPLETE**
