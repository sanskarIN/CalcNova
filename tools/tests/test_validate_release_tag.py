...
class ReleaseTagValidationTests(unittest.TestCase):
    def test_accepts_stable_semantic_versions(self) -> None:
        for tag in ("v0.1.0", "v1.0.0", "v3.4.5", "v10.25.300"):
            with self.subTest(tag=tag):
                self.assertTrue(is_valid_release_tag(tag))

    def test_leading_zero_display_versions_use_normalized_semver_tags(self) -> None:
        self.assertTrue(is_valid_release_tag("v3.4.5"))
        self.assertFalse(is_valid_release_tag("v3.4.05"))
